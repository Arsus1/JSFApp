...
$(() => {
    const $xSlider = PF('widget_x_value');
    const $yText = $('.y-text');
    ...
    checkY = (yValue) => yValue >= -5 && yValue <= 5;
    ...
    setRValid = (isValid, {$rButton}) => {
        if (isValid) {
            $rButton.children().each((index, element) => $(element).removeClass('is-invalid').addClass('is-valid'));
        } else {
            $rButton.children().each((index, element) => $(element).removeClass('is-valid').addClass('is-invalid'));
        }
    };
    ...
    resetRValidity = ($rButton) => {
        $rButton.children().each((index, element) => $(element).removeClass('is-valid is-invalid'));
    };
